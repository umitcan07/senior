...
def krippendorff_alpha_interval(ratings: dict[str, dict[str, float]]) -> tuple[float | None, int]:
    """Krippendorff's alpha with the interval difference metric.

    ratings: clip_id -> {rater: score}. Uses only units rated by >=2 raters.
    alpha = 1 - Do/De, where Do is observed disagreement and De expected
    disagreement, both over all within-unit and all cross-unit pairs using the
    squared-difference (interval) metric. Returns (alpha, n_units_used).
    """
    units = [list(rmap.values()) for rmap in ratings.values() if len(rmap) >= 2]
    n_pairable = sum(len(v) for v in units)
    if n_pairable < 2:
        return None, len(units)

    # Observed disagreement: average squared difference over within-unit pairs,
    # weighted so each unit contributes pairs/(m_u - 1) per the alpha formulation.
    do_num = 0.0
    for vals in units:
        m = len(vals)
        s = sum((vals[i] - vals[j]) ** 2 for i in range(m) for j in range(i + 1, m))
        do_num += (2.0 * s) / (m - 1)
    do = do_num / n_pairable

    # Expected disagreement: average squared difference over all value pairs.
    allv = [x for vals in units for x in vals]
    n = len(allv)
    de_num = sum((allv[i] - allv[j]) ** 2 for i in range(n) for j in range(i + 1, n))
    de = (2.0 * de_num) / (n * (n - 1)) if n > 1 else 0.0
    if de == 0:
        return None, len(units)
...
def main() -> None:
    ...
    # Krippendorff's alpha (interval metric) across all raters, on multiply-rated units
    kalpha, k_units = krippendorff_alpha_interval(ratings)
    ...
    summary = {
        ...
        "krippendorff_alpha_interval": round(kalpha, 4) if kalpha is not None else "n/a (need >=2 raters)",
        "krippendorff_n_units": k_units,
