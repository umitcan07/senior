...
This document outlines the manual steps to deploy the assessment worker to RunPod.
(The V1 IPA-generation endpoint was removed in E1 / #12.)
...
Navigate to the `mod/` directory and build the assessment image.
...
## Step 2: Push the Image to Docker Registry
...
## Step 3: Deploy the Assessment Endpoint on RunPod
...
## Step 5: Update Frontend/API Integration

Update your frontend or API code to use the new endpoint URL (from Step 3).
...
1. **Image not found**: Ensure the image is pushed to the correct registry
...
- Test locally with `docker run` to verify the image works
- Check handler logs for Python errors

## Next Steps

1. **Phone-level alignment**: implement POWSM CTC forced alignment for timestamps (E3 / #19)
2. **Optimize performance**:
   - Pre-load models in handler initialization (already done in `assess.py`)
