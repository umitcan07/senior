...
In parallel, a second perceived-label fine-tune was carried out independently on the full L2-ARCTIC corpus, as a cross-check on the central convention claim. It shares this work's annotation philosophy---it trains exclusively on the perceived (PPL) surface, resolving L2-ARCTIC's \texttt{CPL,PPL,tag} error templates to the sequence a listener actually heard, and never touches the canonical forced-alignment tier---and it reuses the same adapter recipe (LoRA, rank~$32$, $\alpha{=}64$, attention projections only). It differs from the ablation above in three deliberate ways: it uses all six L1 groups under a speaker-disjoint, L1-balanced split (twelve speakers train, six validation, six test, for roughly nine hundred held-out utterances), trains for $25$ epochs, and scores raw phone error rate against the perceived labels without the diacritic-folding step. The intent is not a controlled same-data ablation but a larger, cross-L1 reproduction: an independent test of whether perceived-label adaptation helps on a held-out population an order of magnitude larger than our own dev split. The two studies are therefore complementary rather than directly comparable on a single number, and Section~\ref{sec:finetuning-results} reports them on their own terms.
...
\textbf{[TODO: this $\rho = 0.45$ is preliminary (1 rater, 24 clips). Re-run \texttt{sig/analysis/validation.py} after $\geq$2 raters submit; report the final $\rho$, Krippendorff's $\alpha$ inter-rater agreement, and insert the scatter plot \texttt{doc/figures/scatter\_system\_vs\_human.png}.]}
...
\subsection{Comparison with an Independent L2-ARCTIC Fine-Tune}

The independent perceived-label fine-tune described in Section~\ref{sec:finetuning-method} provides an external check on the convention claim, on a held-out population roughly an order of magnitude larger than our own dev split. On its six-L1, speaker-disjoint test set ($\approx$900 utterances), the perceived-label adapter lowers phone error rate against the perceived labels from a baseline of $0.388$ to $0.192$---a $19.5$~percentage-point reduction---and, crucially, the improvement holds in \emph{every one} of the six first-language groups (Table~\ref{table:collabPerL1}). The per-phone breakdown points in the expected direction: substitution and deletion error counts fall sharply on exactly the segments that distinguish L2 from native English---the American rhotic ``r'' ($705 \rightarrow 161$ errors), the reduced schwa and the lax vowels of \emph{bit} and \emph{cat} (schwa $1100 \rightarrow 447$, lax-i $889 \rightarrow 352$, ash $394 \rightarrow 148$), and the glide ``w'' ($641 \rightarrow 167$)---which is the signature of a model that has learned to transcribe what was produced rather than normalize it to the dictionary form. A small number of segments regress (notably the voiced \emph{th} and ``v''), consistent with the adapter trading a little accuracy on rarer targets for large gains on the dominant error classes.

\begin{table}[h]
\centering
\caption[Independent L2-ARCTIC fine-tune, per-L1 PER]{Per-L1 phone error rate (vs.\ perceived labels) for the independent L2-ARCTIC fine-tune, baseline POWSM vs.\ the perceived-label LoRA adapter, on the six-L1 speaker-disjoint test set. Greedy decoding; PER measured without diacritic folding (see text).}
\label{table:collabPerL1}
...
These figures are not directly comparable to the ablation in Table~\ref{table:ftAblation}: the two studies use different L2-ARCTIC splits, different decoding (greedy here vs.\ beam search), and this run reports raw PER without the phone-folding convention, and it does not measure deviation recall or native drift. What matters is the agreement at the level of the claim. Two perceived-label fine-tunes---ours, a controlled same-data ablation centred on a small Turkish-native set, and this one, a larger cross-L1 reproduction---both improve over the canonical-leaning baseline, and neither finds any benefit in canonical supervision. The convergence of an internally controlled experiment and an independent external one strengthens the central result: the annotation \emph{convention} of the supervision target, not merely the presence of in-domain data, is what governs whether fine-tuning builds a deviation detector or merely sharpens a transcriber.

% TODO (Tier 2): once the collaborator's adapter weights are scored through
% scripts/eval_adapters.py on OUR split/metrics (folded PER, deviation recall,
% native FPR), add a unified row to Table~\ref{table:ftAblation} placing it on the
% same axis as l2a_ppl / l2a_ppl_long, and report the on-our-metrics numbers here.
