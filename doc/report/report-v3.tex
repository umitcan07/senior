...
A model that \emph{detects} mispronunciations must be trained on what the learner actually \emph{produced}, not on the dictionary form of the target word---otherwise it learns to normalize deviations away rather than surface them. This bias originates upstream, in how phonetic foundation models are built: they are trained on \emph{canonical} phonetic transcriptions, and POWSM \cite{powsm} in particular is trained on the IPAPack++ corpus, whose labels are largely produced by \emph{automatic} IPA-transcription methods rather than by humans annotating the surface form they hear. POWSM is therefore, by construction, a canonical transcriber: it was never built to capture accent-level differences or mispronunciations, and emits dictionary-form phones by default---precisely the normalizing behaviour a deviation detector must overcome. This is not a shortcoming peculiar to POWSM: to our knowledge \emph{no} openly available base model performs L2 deviation detection out of the box. It remains an active research area, held back as much by data as by modelling, and the commercial systems that approximate it (e.g.\ ELSA Speak \cite{elsaspeak}) rely on proprietary models and undisclosed data. % TODO: add a citation for IPAPack++ if a suitable reference is available.
...
\textbf{Compute.} Parameter-efficient adaptation keeps the experiment within modest, mostly self-hosted hardware. Data preparation, local development, and the lighter evaluation passes ran on a consumer GPU (NVIDIA GeForce RTX~4060~Ti, 8\,GB); the full adapter sweep and the held-out evaluation matrix ran on a rented RunPod GPU pod when more memory or throughput was needed. The production inference path is unchanged from training: the same fine-tuned adapter is served on the existing RunPod \emph{serverless} GPU endpoint, which scales to zero between requests, so on-demand assessment carries no idle cost.
...
% save it to doc/report/images/screen/v2-intelligibility.png, then replace
% the fbox below with: \includegraphics[width=\textwidth]{images/screen/v2-intelligibility.png}
