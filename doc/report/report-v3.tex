...
These figures are not directly comparable to the ablation in Table~\ref{table:ftAblation}: the two runs use different L2-ARCTIC splits, different decoding (greedy here vs.\ beam search), and this run reports raw PER without the phone-folding convention and does not measure deviation recall or native drift. What matters is the agreement at the level of the claim. Both perceived-label fine-tunes---the controlled same-data ablation centred on the small Turkish-native set, and this larger full-corpus run across all six L1 groups---improve over the canonical-leaning baseline, and neither finds any benefit in canonical supervision. The convergence of the controlled experiment and the scaled-up run strengthens the central result: the annotation \emph{convention} of the supervision target, not merely the presence of in-domain data, is what governs whether fine-tuning builds a deviation detector or merely sharpens a transcriber.

% TODO (Tier 2): once Ömer's adapter weights are scored through
