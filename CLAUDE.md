...
- **Drizzle migrations** — `app/src/db/schema.ts` is the single source of truth. Every schema change: edit schema.ts → `pnpm db:generate` → **inspect the SQL** (if it changes anything you didn't intend, STOP and reconcile drift) → commit the schema change **and** the generated `app/drizzle/NNNN_*.sql` + `meta/NNNN_snapshot.json` + `meta/_journal.json` in **one commit**. Migrations are append-only (never edit/delete an existing migration or snapshot — only exception: make a not-yet-applied migration idempotent). **Deploy applies migrations with `db:migrate`** (Fly `release_command` runs `scripts/migrate.ts`) against the single Neon DB used for local **and** prod, so every committed migration must apply cleanly there. **Never apply DDL to the DB by hand (`ALTER …`) or via `db:push` outside a generated migration** — it desyncs the journal and makes `db:migrate` fail with `already exists` on the next deploy (this bit us: the `0009_chief_tiger_shark` orphan + a manually-added enum value). If an object may already exist from out-of-band history, write the migration with `IF NOT EXISTS`. Full rules: `doc/db.md` §Migrations.
