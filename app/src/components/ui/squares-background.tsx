...
export function Squares({
	...
	useEffect(() => {
		...
		const drawGrid = () => {
			...
			ctx.lineWidth = 0.5;
			ctx.strokeStyle = getResolvedColor(borderColor);
			const fillColor = getResolvedColor(hoverFillColor);
	...
	const handleMouseMove = (event: React.MouseEvent<HTMLCanvasElement>) => {
		...
		setHoveredSquare({
			x: Math.floor(effectiveX / squareSize),
			y: Math.floor(effectiveY / squareSize),
		});
	};
